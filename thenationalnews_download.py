...
bash news_download.sh &


####thenationalnews_download.sh####
while true; do
...
today_folder_page_html = '/data/thenationalnews/page_html/source=%s'%(today)
today_folder_page_list_html = '/data/thenationalnews/page_list_html/source=%s'%(today)
...
list_page_urls = [
	{'page_url':'https://www.thenationalnews.com/business/property/',}
]
...
re_page_url = [
	re.compile(r'\<a href\="(?P<page_url>\/[^\\\/]*?\/[^\\\/]*?\/\d+\/\d+\/\d+\/[^\\\/]*?\/)" class\="', flags=re.DOTALL),
	]

page_url_prefix = 'https://www.thenationalnews.com'
